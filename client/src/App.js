...
import { THCSubdomainResultsModal } from './modals/thcSubdomainModals';
import initiateTHCSubdomainScan from './utils/initiateTHCSubdomainScan';
...
function App() {
  ...
  const [showTHCSubdomainResultsModal, setShowTHCSubdomainResultsModal] = useState(false);
  const [thcSubdomainScans, setTHCSubdomainScans] = useState([]);
  const [mostRecentTHCSubdomainScanStatus, setMostRecentTHCSubdomainScanStatus] = useState(null);
  const [mostRecentTHCSubdomainScan, setMostRecentTHCSubdomainScan] = useState(null);
  const [isTHCSubdomainScanning, setIsTHCSubdomainScanning] = useState(false);
  ...
  const handleDelete = async () => {
    ...
    try {
      const response = await fetch(`${process.env.REACT_APP_SERVER_PROTOCOL}://${process.env.REACT_APP_SERVER_IP}/scopetarget/delete/${activeTarget.id}`, {
        method: 'DELETE',
      });
  ...
  const startTHCSubdomainScan = () => {
    initiateTHCSubdomainScan({
      activeTarget,
      setIsTHCSubdomainScanning,
      setMostRecentTHCSubdomainScan,
      setMostRecentTHCSubdomainScanStatus
  ...
  const handleCloseTHCSubdomainResultsModal = () => setShowTHCSubdomainResultsModal(false);
  const handleOpenTHCSubdomainResultsModal = () => setShowTHCSubdomainResultsModal(true);
    ...
    try {
      ...
      const updatePromises = safeData.map(async (target) => {
        ...
        const updateResponse = await fetch(
          `${process.env.REACT_APP_SERVER_PROTOCOL}://${process.env.REACT_APP_SERVER_IP}/api/target-urls/${target.id}/roi-score`,
          {
            method: 'PUT',
            headers: {
              'Content-Type': 'application/json',
  ...
  return (
    <Container data-bs-theme="dark" className="App" style={{ padding: '20px' }}>
      ...
      <ReconResultsModal
        ...
        thcSubdomainResults={mostRecentTHCSubdomainScan}
      ...
      <THCSubdomainResultsModal
        showTHCSubdomainResultsModal={showTHCSubdomainResultsModal}
        handleCloseTHCSubdomainResultsModal={handleCloseTHCSubdomainResultsModal}
        thcSubdomainResults={mostRecentTHCSubdomainScan}
      />
                ...
                <Row className="justify-content-between mb-4">
                  {[
                    { 
                      ...
                      description: 'A fast web spider written in Go for web scraping and crawling.',
                      isActive: true,
                    ...
                    { 
                      ...
                      description: 'Extract subdomains from JavaScript files and external sources.',
                      isActive: true,
                    ...
                    { 
                      ...
                      description: 'Download subdomains from THC\'s comprehensive subdomain database.',
                      isActive: true,
                      status: mostRecentTHCSubdomainScanStatus,
                      isScanning: isTHCSubdomainScanning,
                      onScan: startTHCSubdomainScan,
                      onResults: handleOpenTHCSubdomainResultsModal,
                      resultCount: mostRecentTHCSubdomainScan && mostRecentTHCSubdomainScan.result ? 
                        mostRecentTHCSubdomainScan.result.split('\n').filter(line => line.trim()).length : 0
                    }
                  ].map((tool, index) => (
                    <Col md={6} className="mb-4" key={index}>
                      <Card className="shadow-sm h-100 text-center" style={{ minHeight: '150px' }}>
