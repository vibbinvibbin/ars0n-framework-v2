const monitorLinkFinderURLScanStatus = async (
  ...
  try {
    const response = await fetch(
      `${process.env.REACT_APP_SERVER_PROTOCOL}://${process.env.REACT_APP_SERVER_IP}/scopetarget/${activeTarget.id}/scans/linkfinder-url`
    );
...
export const monitorActiveScan = async (
  ...
  const poll = async () => {
    try {
      const response = await fetch(
        `${process.env.REACT_APP_SERVER_PROTOCOL}://${process.env.REACT_APP_SERVER_IP}/linkfinder-url/status/${scanId}`
      );
